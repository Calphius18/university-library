...
const AuthForm = <T extends FieldValues>({
  ...
  const handleSubmit: SubmitHandler<T> = async (data) => {
    ...
    if(result.success) {
      ...
      toast.error(`Error ${isSignIn ? "signing in" : "signing out"}`, {
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-2xl font-semibold text-white">
        {isSignIn ? "Welcome back to CaliBooks" : "Create your library account"}
      </h1>

      <p className="text-light-100">
        {isSignIn
          ? "Access the plethora of resources in the citadel of knowledge, and stay updated"
      ...
      <p className="text-center text-base font-medium">
        {isSignIn ? "New to CaliBooks? " : "Already have an account? "}
        <Link
